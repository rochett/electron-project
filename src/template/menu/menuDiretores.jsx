...
import { faPlus, faList, faVideo, faBullhorn } from '@fortawesome/free-solid-svg-icons';
...
export default function MenuDiretores({ lastMovieTag }) {
  ...
  const tableData = 'Diretores';

  return (
    <>
      <Card bg="dark" text="white" border="info">
        <Card.Header>
          {titulo_secao.diretor.titulo}<FontAwesomeIcon icon={faBullhorn} className="Icon-Menu" />
        </Card.Header>
        ...
        <Modal.Body>
          {
            showList === true ? <ListaGeral tableData={removeAccents(tableData).toLowerCase()} titulos={titles} /> :
              <FormAtorDirRot tableData={removeAccents(tableData).toLowerCase()} titleTag="Diretor" idForm="-1" dadosReg="" />
          }
        </Modal.Body>
      </Modal>

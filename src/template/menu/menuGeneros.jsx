...
import { faPlus, faList, faVideo, faUserAstronaut } from '@fortawesome/free-solid-svg-icons';
...
export default function MenuGeneros({ lastMovieTag }) {
  ...
  const tableData = 'Gêneros';

  return (
    <>
      <Card bg="dark" text="white" border="info">
        <Card.Header>
          {titulo_secao.genero.titulo}<FontAwesomeIcon icon={faUserAstronaut} className="Icon-Menu" />
        </Card.Header>
        ...
        <Modal.Body>
          {
            showList === true ? <ListaGeral tableData={removeAccents(tableData).toLowerCase()} titulos={titles} /> :
              <FormGenPre tableData={removeAccents(tableData).toLowerCase()} titleTag="Gênero" idForm="-1" dadosReg="" />
          }
        </Modal.Body>
      </Modal>
